...
const req = http.request(apiUrl, options, (res) => {
  ...
  res.on("end", () => {
    ...
    // CSV-Daten erstellen
    const csvData = jsonData.elements
      .filter((element) => {
        return (
          element.type == "node" && element.tags.emergency == "fire_hydrant"
        );
      })
      .map((element) => {
        let description = [];
        let row = [0, element.lat, element.lon];
        switch (element.tags["fire_hydrant:type"]) {
          case "underground":
            ...
            if (element.tags["fire_hydrant:pressure"] == "0") {
              row[0] = 666;
            }
            break;
          case "pillar":
        ...
        if (element.tags.description)
          description.push(element.tags.description);
        if (element.tags.position == "lane") description.push("Fahrbahn");
        if (element.tags.position == "parking_lot")
          description.push("Parkbucht");
        if (element.tags.position == "sidewalk")
          description.push("Bürgersteig");
